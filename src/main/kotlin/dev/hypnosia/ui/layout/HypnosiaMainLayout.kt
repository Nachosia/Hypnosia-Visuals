...
object HypnosiaMainLayout {
    ...
    private class ModuleSettingsDrawerNode(
        ...
        private fun renderColorPicker(context: DrawContext) {
            ...
            drawButton(context, x + 10.0f, y + 10.0f, 64.0f, 30.0f, "Close")
            HypnosiaRenderUtils.drawThemedBox(context, x + 188.0f, y + 10.0f, 126.0f, 30.0f, 9.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.BUTTON)
            HypnosiaRenderUtils.drawFigmaBox(context, x + 198.0f, y + 18.0f, 14.0f, 14.0f, 7.0f, color)
            drawText(context, hexNoAlpha(color), x + 220.0f, y + 17.0f, 12.0f, 0xFFBFC0CA.toInt())

            drawText(context, "Color Picker ${target.label}", x + 10.0f, y - 13.0f, 10.0f, 0xFF8E8E98.toInt())
            HypnosiaRenderUtils.drawThemedBox(context, x + 10.0f, y + 50.0f, 94.0f, 30.0f, 7.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.BUTTON)
            drawText(context, "Fine Tuning", x + 22.0f, y + 59.0f, 12.0f, 0xFFBFC0CA.toInt())
            HypnosiaRenderUtils.drawThemedBox(context, x + 104.0f, y + 50.0f, 116.0f, 30.0f, 7.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.BUTTON)
            drawText(context, "HSV + Alpha", x + 119.0f, y + 59.0f, 12.0f, 0xFFE7E7EA.toInt())
            ...
            HypnosiaRenderUtils.drawFigmaBox(context, markerX - 5.0f, markerY - 5.0f, 10.0f, 10.0f, 5.0f, 0x00000000, WHITE, 2.0f)

            val hue = pickerHueRect()
            drawText(context, "Hue", x + 18.0f, hue.y - 18.0f, 11.0f, 0xFF8E8E98.toInt())
            HypnosiaRenderUtils.drawHueStrip(context, hue.x, hue.y, hue.width, hue.height, 5.0f)
            HypnosiaRenderUtils.drawFigmaBox(context, hue.x + hsv[0] * hue.width - 4.0f, hue.y - 2.0f, 8.0f, hue.height + 4.0f, 4.0f, WHITE)

            val opacity = pickerOpacityRect()
            drawText(context, "Opacity", x + 18.0f, opacity.y - 18.0f, 11.0f, 0xFF8E8E98.toInt())
            HypnosiaRenderUtils.drawAlphaStrip(context, opacity.x, opacity.y, opacity.width, opacity.height, 5.0f, color)
            HypnosiaRenderUtils.drawFigmaBox(context, opacity.x + alpha * opacity.width - 4.0f, opacity.y - 2.0f, 8.0f, opacity.height + 4.0f, 4.0f, WHITE)

            val valueY = y + 372.0f
            HypnosiaRenderUtils.drawThemedBox(context, x + 18.0f, valueY, 54.0f, 32.0f, 7.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.BUTTON)
            drawText(context, "HEX", x + 34.0f, valueY + 10.0f, 12.0f, 0xFFE7E7EA.toInt())
            HypnosiaRenderUtils.drawThemedBox(context, x + 82.0f, valueY, 158.0f, 32.0f, 7.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.INPUT)
            drawText(context, hexNoAlpha(color), x + 98.0f, valueY + 10.0f, 12.0f, 0xFFBFC0CA.toInt())
            HypnosiaRenderUtils.drawThemedBox(context, x + 250.0f, valueY, 64.0f, 32.0f, 7.0f, DRAWER_BG, DRAWER_STROKE, 1.0f, ThemeSettings.ThemeRole.INPUT)
            drawText(context, "${(alpha * 100.0f).toInt()}%", x + 270.0f, valueY + 10.0f, 12.0f, 0xFFE7E7EA.toInt())

            drawText(context, "Menu Palette", x + 18.0f, y + 418.0f, 11.0f, 0xFF8E8E98.toInt())
            colorPickerSwatches().forEach { (swatch, rect) ->
                val selected = (color and 0x00FFFFFF) == (swatch and 0x00FFFFFF)
                HypnosiaRenderUtils.drawFigmaBox(context, rect.x, rect.y, rect.width, rect.height, 4.0f, swatch, if (selected) WHITE else 0x00000000, if (selected) 2.0f else 0.0f)
            }
        }
        ...
        private fun handleColorPickerClick(mouseX: Float, mouseY: Float): Boolean {
            ...
            val drag = when {
                ...
                contains(mouseX, mouseY, pickerHueRect().x, pickerHueRect().y - 4.0f, pickerHueRect().width, pickerHueRect().height + 8.0f) -> ColorPickerDrag.HUE
                contains(mouseX, mouseY, pickerOpacityRect().x, pickerOpacityRect().y - 4.0f, pickerOpacityRect().width, pickerOpacityRect().height + 8.0f) -> ColorPickerDrag.OPACITY
                else -> null
            }
            if (drag != null) {
